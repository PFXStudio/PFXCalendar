    ...
    func bindOutputs() {
        ...
        self.rxDataSource = RxCollectionViewSectionedAnimatedDataSource<RxSectionCollectionViewModel>(configureCell: { dataSource, collectionView, indexPath, cellViewModel in
            ...
            guard let viewModel = try? (dataSource.model(at: indexPath) as? RxCellViewModel),
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: viewModel.reuseIdentifier, for: indexPath) as? CalendarCell else {
                    assert(true)
            ...
            return cell
                ...
                if kind == UICollectionView.elementKindSectionHeader {
                    guard let cell = cv.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: String(describing: CalendarHeaderCell.self), for: ip) as? CalendarHeaderCell else {
