...
class CalendarCollectionViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        self.viewModel = CalendarCollectionViewModel(dependency: CalendarCollectionViewModel.Dependency())
        self.bindOutputs()
        let startDate = Date()
...
//        button.setImage(UIImage(systemName: "xmark"), for: .normal)
    ...
    func bindOutputs() {
        ...
        self.rxDataSource = RxCollectionViewSectionedAnimatedDataSource<RxSectionCollectionViewModel>(configureCell: { dataSource, collectionView, indexPath, cellViewModel in
            ...
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: viewModel.reuseIdentifier, for: indexPath) as? CalendarCell {
                cell.configure(viewModel: viewModel, indexPath: indexPath)
                return cell
            }
            
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: viewModel.reuseIdentifier, for: indexPath) as? CalendarEmptyCell {
                return cell
            }
...
//        let width = (self.collectionView.frame.width - 32 - 66 - (8.3 * 7)) / numberOfColums
        ...
        if viewModel is HorizontalCalendarCellViewModel || viewModel is CalendarEmptyCellViewModel {
